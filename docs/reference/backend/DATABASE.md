...
**Purpose:** Tauri-specific file persistence for the sql.js database. Loads and saves the database binary to the Tauri AppData directory (`Junban/junban.db`) only for runtimes that still own persistence locally; remote-desktop browsers are hard-blocked from direct database file access and must go through the backend API.
**Key Exports:**
