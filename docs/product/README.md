...
Product docs live under `docs/product/` so mission, roadmap, status, and PRD-style planning stay separate from the contributor/codebase docs library in `docs/guides/` and `docs/reference/`.
...
Create or update a product PRD when all of the following are true:

1. The work is a roadmap-level milestone or materially changes product positioning/capability.
2. Scope and user-facing outcome are stable enough to describe without task-level delivery detail.
3. The milestone is expected to span multiple execution items (not a single backlog ticket).

PRDs should capture problem, target outcome, in/out scope, and dependencies without sprint plans, ticket inventories, or delivery history.
