...
> The theme system: manager, CSS files, and configuration.
...
### src/ui/themes/manager.ts (90 lines)
...
### src/ui/themes/light.css (27 lines)
...
### src/ui/themes/dark.css (22 lines)
...
### src/ui/themes/nord.css (22 lines)
...
The accent color can be overridden by the user via Settings > Appearance. The `SettingsContext` applies a custom accent color by directly setting CSS custom properties:
...
The `darkenColor` helper (in `SettingsContext.tsx`) converts hex to HSL, reduces lightness by the given amount, and converts back to hex. This ensures the hover state is always slightly darker than the base accent regardless of which color the user picks.
