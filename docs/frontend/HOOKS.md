...
> Every custom hook in `src/ui/hooks/` and view-specific hooks in `src/ui/views/`.
...
- **Path:** `src/ui/hooks/useTaskHandlers.ts` (125 lines)
...
- **Path:** `src/ui/hooks/useKeyboardNavigation.ts` (72 lines)
...
- **Path:** `src/ui/hooks/useMultiSelect.ts` (53 lines)
...
- **Path:** `src/ui/hooks/useBulkActions.ts` (52 lines)
...
- **Path:** `src/ui/hooks/useAppShortcuts.ts` (88 lines)
...
- **Registered Shortcuts:**
  ...
  - `Ctrl+Shift+D` -- Toggle dark mode (via `themeManager.toggle()`)
  - `Ctrl+Z` -- Undo
  - `Ctrl+Shift+Z` -- Redo
  - `Ctrl+F` -- Toggle search (if `setSearchOpen` provided)
  - `Ctrl+Shift+F` -- Enter focus mode (if `setFocusModeOpen` provided)
  - `Q` -- Quick add task (if `setQuickAddOpen` provided)
...
- **Path:** `src/ui/hooks/useAppCommands.ts` (130 lines)
...
- **Path:** `src/ui/hooks/useIsMobile.ts` (20 lines)
...
- **Path:** `src/ui/hooks/useSoundEffect.ts` (31 lines)
...
- **Path:** `src/ui/hooks/useReminders.ts` (52 lines)
...
- **Path:** `src/ui/hooks/useVAD.ts` (203 lines)
...
- **Path:** `src/ui/hooks/useVoiceCall.ts` (198 lines)
...
- **Path:** `src/ui/hooks/useFocusTrap.ts` (51 lines)
